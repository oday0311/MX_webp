...
static MV_WEBP_INLINE void VE8uv(uint8_t* dst, const uint8_t* top) {
  int j;
...
static MV_WEBP_INLINE void VE16(uint8_t* dst, const uint8_t* top) {
...
static MV_WEBP_INLINE void HE8uv(uint8_t* dst, const uint8_t* left) {
  int j;
  for (j = 0; j < 8; ++j) {
...
static MV_WEBP_INLINE void HE16(uint8_t* dst, const uint8_t* left) {
  int j;
  for (j = 0; j < 16; ++j) {
    const __m128i values = _mm_set1_epi8(left[j]);
...
static MV_WEBP_INLINE void TM(uint8_t* dst, const uint8_t* left,
                           const uint8_t* top, int size) {
  const __m128i zero = _mm_setzero_si128();
  int y;
  if (size == 8) {
...
static MV_WEBP_INLINE void DC8uv(uint8_t* dst, const uint8_t* left,
                              const uint8_t* top) {
  const __m128i top_values = _mm_loadl_epi64((const __m128i*)top);
...
static MV_WEBP_INLINE void DC8uvNoLeft(uint8_t* dst, const uint8_t* top) {
  const __m128i zero = _mm_setzero_si128();
  const __m128i top_values = _mm_loadl_epi64((const __m128i*)top);
...
static MV_WEBP_INLINE void DC8uvNoTop(uint8_t* dst, const uint8_t* left) {
  // 'left' is contiguous so we can reuse the top summation.
  DC8uvNoLeft(dst, left);
}

static MV_WEBP_INLINE void DC8uvNoTopLeft(uint8_t* dst) {
  Put8x8uv(0x80, dst);
}

static MV_WEBP_INLINE void DC8uvMode(uint8_t* dst, const uint8_t* left,
                                  const uint8_t* top) {
  if (top != NULL) {
...
static MV_WEBP_INLINE void DC16(uint8_t* dst, const uint8_t* left,
                             const uint8_t* top) {
...
static MV_WEBP_INLINE void DC16NoLeft(uint8_t* dst, const uint8_t* top) {
  const __m128i top_row = _mm_load_si128((const __m128i*)top);
  const int DC = VP8HorizontalAdd8b(&top_row) + 8;
  Put16(DC >> 4, dst);
}

static MV_WEBP_INLINE void DC16NoTop(uint8_t* dst, const uint8_t* left) {
  // 'left' is contiguous so we can reuse the top summation.
  DC16NoLeft(dst, left);
}

static MV_WEBP_INLINE void DC16NoTopLeft(uint8_t* dst) {
  Put16(0x80, dst);
}

static MV_WEBP_INLINE void DC16Mode(uint8_t* dst, const uint8_t* left,
                                 const uint8_t* top) {
  if (top != NULL) {
    if (left != NULL) {  // top and left present
      DC16(dst, left, top);
...
static MV_WEBP_INLINE void VE4(uint8_t* dst, const uint8_t* top) {  // vertical
...
static MV_WEBP_INLINE void HE4(uint8_t* dst, const uint8_t* top) {  // horizontal
  const int X = top[-1];
  const int I = top[-2];
  const int J = top[-3];
  const int K = top[-4];
...
static MV_WEBP_INLINE void DC4(uint8_t* dst, const uint8_t* top) {
  uint32_t dc = 4;
  int i;
...
static MV_WEBP_INLINE void LD4(uint8_t* dst, const uint8_t* top) {  // Down-Left
  const __m128i one = _mm_set1_epi8(1);
...
static MV_WEBP_INLINE void VR4(uint8_t* dst,
...
static MV_WEBP_INLINE void VL4(uint8_t* dst,
...
static MV_WEBP_INLINE void RD4(uint8_t* dst, const uint8_t* top) {  // Down-right
  const __m128i one = _mm_set1_epi8(1);
...
static MV_WEBP_INLINE void HU4(uint8_t* dst, const uint8_t* top) {
  const int I = top[-2];
  const int J = top[-3];
  const int K = top[-4];
  const int L = top[-5];
...
static MV_WEBP_INLINE void HD4(uint8_t* dst, const uint8_t* top) {
  const int X = top[-1];
  const int I = top[-2];
  const int J = top[-3];
  const int K = top[-4];
...
static MV_WEBP_INLINE void TM4(uint8_t* dst, const uint8_t* top) {
  const __m128i zero = _mm_setzero_si128();
...
static MV_WEBP_INLINE void SubtractAndAccumulate(const __m128i a, const __m128i b,
                                              __m128i* const sum) {
  // take abs(a-b) in 8b
...
static MV_WEBP_INLINE int SSE_16xN(const uint8_t* a, const uint8_t* b,
                                int num_pairs) {
